...
declare -a samples=(0 5 8 9 8 4 0)
echo "--------" >> sampler1.log
echo "--------" >> sampler1.sig
while $alive; do
  for sample in "${samples[@]}"; do
    ...
    for (( i=0; i<${#containers[@]}; i++ )); do
      ...
      (if docker exec -d $container stress --cpu 2 --timeout ${sample}s --verbose ; then
        printf -v ts '%(%s)T' -1
        echo "docker exec -d $container stress --cpu 2 --timeout ${sample}s --verbose #$ts" >> sampler1.log
